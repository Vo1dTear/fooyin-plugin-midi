...
void configurePlayer(SpessaPlayer* player, QString fileBank, bool is_gs)
{
    ...
    double gain = setting.value(GainSetting, DefaultGain).toDouble();
...
std::optional<Fooyin::AudioFormat> MIDIDecoder::init(
    const Fooyin::AudioSource& source,
    ...
    DecoderOptions options)
{
    ...
    ss_midi_load(
        midiFile,
        track.extension().toUtf8().constData()
    );
    ...
    const bool repeatTrack =
    !(options & NoInfiniteLooping) &&
    isRepeatingTrack();
    ...
    if(track.isInArchive() && source.archiveReader) {
        ...
        for(const auto ext : soundfontExtensions()) {
            ...
            fileInfo.dir().relativeFilePath(
                fileInfo.completeBaseName() + u"."_s + ext
            );
            
            auto soundfontEntry =
            source.archiveReader->entry(soundfontPath);
            
            if(soundfontEntry.device) {
                const auto soundfontData =
                soundfontEntry.device->readAll();
                
                if(!soundfontData.isEmpty()) {
                    spessaplayer->setFileSoundFontData(
                        reinterpret_cast<const uint8_t*>(
                            soundfontData.constData()
                        ),
                        soundfontData.size()
                    );
        ...
        if(!found) {
            ...
            for(const auto ext : soundfontExtensions()) {
                const QString soundfontPath =
                fileInfo.dir().relativeFilePath(
                    infoarchive.completeBaseName() + u"."_s + ext
                );
    ...
    const double subsong_begin =
    subsong_start_seconds(
        m_midiFile,
        static_cast<size_t>(subsong)
    );
    
    const double subsong_end =
    subsong_end_seconds(
        m_midiFile,
        static_cast<size_t>(subsong)
    );
    ...
    if(m_midiFile->loop.end > 0) {
        loopStart =
        ss_midi_ticks_to_seconds(
            m_midiFile,
            m_midiFile->loop.start
        );
    ...
    const bool hasLoop =
        m_midiFile->loop.end > m_midiFile->loop.start &&
        loopEnd > loopStart &&
    ...
    if(hasLoop) {
        if(loopCount > 0) {
            ...
            *   INTRO -> LOOP -> FINAL
            *
            * Each loop count adds another LOOP section pass.
            *
            * Therefore:
            *
            *   0 = INTRO -> LOOP -> FINAL
            *   1 = INTRO -> LOOP -> LOOP -> FINAL
            *   2 = INTRO -> LOOP -> LOOP -> LOOP -> FINAL
            */
            const double loopLength =
            loopEnd - loopStart;
            
            const double finalLength =
            framesLength - loopEnd;
            
            framesLength =
            loopStart +
            (loopLength * (loopCount + 1)) +
            finalLength;
        }
    ...
    const unsigned int loop_mode =
    repeatOne
    ? MIDIPlayer::loop_mode_enable |
    ...
    m_midiPlayer->setLoopCount(
        isLooped
        ? static_cast<unsigned int>(loopCount)
        : 0
    );
    
    if(!m_midiPlayer->Load(
        m_midiFile,
        static_cast<unsigned>(subsong),
                        loop_mode,
                        playbackFade,
                        loopStart,
                        loopEnd,
    ...
    /*
    * SpessaSynth's loop count is configured after Load(),
    * because Load() resets the sequencer loop count.
    *
    * The UI value is the number of additional loop jumps requested from
    * SpessaSynth. The explicit playback duration above limits the rendered
    * output to the duration reported by the decoder.
    */
    m_midiPlayer->setLoopCount(
        isLooped
        ? static_cast<unsigned int>(loopCount)
    ...
    m_midiPlayer->setFilterMode(
        MIDIPlayer::filter_default,
        false
    );
...
bool MIDIReader::readTrack(const Fooyin::AudioSource& source, Fooyin::Track& track) {
    ...
    double framesLength =
    subsong_end - subsong_begin;
    
    framesLength =
    round(framesLength * 1000.0);
    
    long totalFrames = framesLength;
    ...
    const SS_RMIDIInfo* ri =
    &m_midiFile->rmidi_info;
