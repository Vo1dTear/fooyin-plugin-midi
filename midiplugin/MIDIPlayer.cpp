...
void MIDIPlayer::setGainDb(double gain_db) {
	gain_db = std::clamp(gain_db, -12.0, 12.0);
	gain = static_cast<float>(std::pow(10.0, gain_db / 20.0));
}
...
void MIDIPlayer::setLoopCount(unsigned int jumpCount) {
	...
	double content_duration = has_playback_duration
		? playback_duration_seconds
		: duration_seconds;
	
	if(!has_playback_duration && loop_count > 0 &&
		loop_end_seconds > loop_start_seconds)
	{
		const double loop_length =
			loop_end_seconds - loop_start_seconds;
		
		content_duration =
			loop_start_seconds +
			(loop_length * (loop_count + 1)) +
			(duration_seconds - loop_end_seconds);
	}
	
	samples_total =
		(long)std::llround(content_duration * dSampleRate);
	
	if(fade_total_samples > 0) {
		fade_begin_sample =
			std::max<long>(0, samples_total - fade_total_samples);
	} else {
		fade_begin_sample = 0;
	}
}
...
static double subsong_end_ticks(const SS_MIDIFile *midi, size_t subsong) {
	if(!midi || midi->format != 2) return 0.0;
	if(subsong >= midi->track_count) return 0.0;
...
bool MIDIPlayer::Load(
	...
	if(midi_file->format != 2) {
		subsong_start_seconds = 0.0;
		subsong_end_seconds = midi_file->duration;
	} else {
	...
	this->loop_start_seconds = loop_start_seconds;
	this->loop_end_seconds = loop_end_seconds;
	this->playback_duration_seconds = playback_duration_seconds;
	has_playback_duration = playback_duration_seconds > 0.0;
	...
	fade_start_sample = 0;
	fade_total_samples =
		(long)std::llround(fade_seconds * dSampleRate);
	fade_begin_sample = 0;
...
bool MIDIPlayer::buildSequencer() {
	...
	int loop_count =
		(loop_mode_flags & loop_mode_enable)
		? -1
		: this->loop_count;
	
	ss_sequencer_set_loop_count(
		sequencer,
		loop_count
	);
	
	ss_sequencer_set_loop_end_behavior(
		sequencer,
	...
	while(done < count) {
		...
		if(fade_total_samples > 0) {
			...
			for(uint32_t i = 0; i < chunk; ++i) {
				...
				if(sample < fade_begin_sample)
					continue;
				
				const long fade_sample =
				sample - fade_begin_sample;
				
				float gain =
				1.0f -
				static_cast<float>(fade_sample) /
				static_cast<float>(fade_total_samples);
...
void MIDIPlayer::Seek(unsigned long sample) {
	...
	if(loop_count > 0 && loop_end_seconds > loop_start_seconds) {
		const double loop_length = loop_end_seconds - loop_start_seconds;
		const double looped_duration = loop_length * (loop_count + 1);

		if(target_seconds >= loop_start_seconds &&
			target_seconds < loop_start_seconds + looped_duration) {
			target_seconds =
				loop_start_seconds +
				std::fmod(target_seconds - loop_start_seconds, loop_length);
		} else if(target_seconds >= loop_start_seconds + looped_duration) {
			target_seconds += loop_end_seconds -
				(loop_start_seconds + looped_duration);
		}
	}
